...
bool cycleDetectionTest() {
    ...
    if (result != CIRCULAR_DEPENDENCY) {
        success = false;
        fprintf(stderr, "Failed on test A1 = A1\n");
        fprintf(stderr, "Expected: CIRCULAR_DEPENDENCY, Got: %d\n", result);
    }
    reset();
    ...
    if (result != CIRCULAR_DEPENDENCY) { // C1 = A1 (creates cycle)
        success = false;
        fprintf(stderr, "Failed on test A1 = B1, B1 = C1, C1 = A1\n");
        fprintf(stderr, "Expected: CIRCULAR_DEPENDENCY, Got: %d\n", result);
    }
    reset();
    ...
    if (result != CIRCULAR_DEPENDENCY) { // D2 = A2 (creates cycle)
        success = false;
        fprintf(stderr, "Failed on test A2 = B2, B2 = C2, C2 = D2, D2 = A2\n");
        fprintf(stderr, "Expected: CIRCULAR_DEPENDENCY, Got: %d\n", result);
    }
    reset();
    ...
    if (result != CIRCULAR_DEPENDENCY) { // range cycle
        success = false;
        fprintf(stderr, "Failed on test range cycle detection\n");
        fprintf(stderr, "Expected: CIRCULAR_DEPENDENCY, Got: %d\n", result);
    }
    reset();
...
bool functionTests() {
    ...
    if (x != 3 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test B1 + C1 = 3\n");
        fprintf(stderr, "Expected: 3 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 5 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test 10 - 5 = 5\n");
        fprintf(stderr, "Expected: 5 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 8 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test 4 * 2 = 8\n");
        fprintf(stderr, "Expected: 8 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 4 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test 8 / 2 = 4\n");
        fprintf(stderr, "Expected: 4 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 42 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test B10 * C10 = 42\n");
        fprintf(stderr, "Expected: 42 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 9 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test SUM(B5:D5) = 9\n");
        fprintf(stderr, "Expected: 9 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 3 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test AVG(B5:D5) = 3\n");
        fprintf(stderr, "Expected: 3 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 1 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test MIN(B5:D5) = 1\n");
        fprintf(stderr, "Expected: 1 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 5 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test MAX(B5:D5) = 5\n");
        fprintf(stderr, "Expected: 5 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    ...
    if (x != 2 || err != NO_ERROR) {
        fprintf(stderr, "Failed on test STDEV(B5:D5) = 2\n");
        fprintf(stderr, "Expected: 2 with NO_ERROR, Got: %d with error %d\n", x, err);
        success = false;
    }
    reset();
...
bool div0Test() {
    ...
    if (err != DIVIDE_BY_ZERO) {
        fprintf(stderr, "Failed on test 1/0\n");
        fprintf(stderr, "Expected: DIVIDE_BY_ZERO error, Got: error %d with value %d\n", err, value);
        success = false;
    }
    ...
    if (err == NO_ERROR) {
        fprintf(stderr, "Failed on test B1 = A1, did not show a divide by 0\n");
        fprintf(stderr, "Expected: DIVIDE_BY_ZERO error, Got: NO_ERROR with value %d\n", value);
        success = false;
    }
    reset();
...
bool runSheetTests() {
    ...
    reset();
    fprintf(stderr, "Starting function tests\n");
    reset();
    success &= functionTests();
    fprintf(stderr, "Starting divide by 0 test\n");
    reset();
    success &= div0Test();
